...
This site is set up as a Georgian-first static editorial biography for ARCI, an architecture and design firm founded in 1989.
...
- Content: JSON files in `/content` for site config, architecture projects, and long-form timeline entries. This keeps editing simple before CMS migration.
...
- Images: `next/image` with remote Unsplash placeholders configured in `next.config.mjs`. Replace these with uploaded black-and-white firm archive photography when ready.
...
- `project`: repeatable document with slug, title, era, year, summary, image, and link fields.
